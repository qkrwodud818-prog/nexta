...
app.post("/api/demo/script", requireAdminUser, async (req, res) => {
  ...
  const prompt =
    ...
    "이번에 홍보할 기능: " + feature + "\n\n" +
    "이 기능을 쓰는 과정을 " + DEMO_SCENES_MAX + "장면으로 쪼개 세로 영상(릴스/숏츠) 대본을 만든다.\n\n" +

    /* 첫 장면이 전부다. 보는 사람은 1.7초 안에 넘길지 말지 정하고, 훅이 약하면
       플랫폼이 아예 노출을 안 준다. 그래서 1번 장면만 규칙을 따로 준다. */
    "[1번 장면 = 훅] 화면 전체에 이 문장 하나만 크게 뜬다. 12자 이내로, 아래 중 한 형태로 쓴다:\n" +
    "  · 통념 뒤집기 — \"매일 올리라고요? 저는 안 올립니다\"\n" +
    "  · 실수 고백 — \"이거 몰라서 3주를 날렸습니다\"\n" +
    "  · 숫자 — \"카드뉴스 5장, 8분\"\n" +
    "  · 질문 — \"아직 직접 쓰세요?\"\n" +
    "숫자가 들어가면 더 잘 걸린다. 단, 지어낸 숫자는 절대 쓰지 않는다 —\n" +
    "제품이 실제로 하는 일에서 나온 숫자만 쓴다(예: 비비 12명, 카드뉴스 3장).\n" +
    "\"이것만 알면 인생이 바뀝니다\" 같은 막연한 문장은 금지다. 인사·자기소개로 시작하지 않는다.\n\n" +

    "[2번 이후] 각 장면은 title(화면 상단 한 줄, 14자 이내)과 narration(하단 자막, 40자 이내)을 갖는다.\n" +
    ...
    "1번 장면은 ui와 uiLines를 비워 둔다.\n" +
    "마지막 장면은 다음 행동을 한 줄로 말한다(예: \"프로필 링크에서 바로 써보세요\").\n\n" +

    "featureLine은 캡션 맨 아래 들어갈 기능 한 줄 설명(30자 이내)이다.\n\n" +
    "훅에서 약속한 것과 뒤 장면의 내용이 어긋나면 안 된다 — 그게 제일 크게 신뢰를 깎는다.\n" +
